...
<main class="bg-gray-50 dark:bg-gray-900 h-screen">
  <div class="flex flex-col items-center justify-center px-6 pt-8 mx-auto h-screen pt:mt-0 dark:bg-gray-900">
    ...
    <div class="p-4 mb-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400 w-full max-w-xl" role="alert" v-if="errorMessage">
        <span class="font-medium">{{$t('error.error')}}:</span> {{ errorMessage }}
    </div>
    <div class="w-full max-w-xl p-6 space-y-8 sm:p-8 bg-white rounded-lg dark:bg-gray-800">
       
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">
            {{ $t('signin.signinto') }} <b>Sasanqua</b> 
        </h2>
        <form class="mt-8 space-y-6" @submit.prevent="submit">
            <div>
                <label for="username" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ $t('signin.username') }}</label>
            ...
            <div>
                <label for="password" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ $t('signin.password') }}</label>
            ...
            <div class="flex items-center">
                <router-link to="/signup" class="text-sm text-primary-700 hover:underline dark:text-primary-500">{{ $t('signin.noaccount')}}</router-link>
                <button type="submit" class="ml-auto w-full px-4 float-right py-2 text-base font-medium text-center text-white bg-primary-700 rounded-lg hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 sm:w-auto dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">{{ $t('signin.signin') }}</button>
            </div>
